...
Options: 
         ...
         -c STR chromosome to run rid (all or chrX, etc) (CURRENTLY IGNORED)
         ...
         -P STR extra directories to add to \$PATH

};

my %opts = ();
getopts("hSc:s:q:d:l:k:n:r:s:p:a:etb:fg:i:P:", \%opts);
...
my $q = "all_12h";
# XXX: $bdir is used to create $dir.  Its only other use is in the R script call
my $bdir = "/files/CBMI/parklab/alee/ra2";
my $max_mem = "2000000000";
my $l = 40;
my $k = 2;
my $n = 3;
# XXX: $ra is always used with basename(), so the path doesn't matter at all
...
$add_path = $opts{P} if (defined($opts{P}));
$tea = dirname($add_path);
...
print "#!$bash_binary\n";
print "PATH=\$PATH:$add_path\n";

################################################################################
# Joe:
#  This whole first section makes sure the 3 required input files exist
#  in $dir/bam.  If not, attempt to copy the files from $sourcedir (-b
#  option).  The 3 required input files are:
#
#    sample.disc.bam    (sample.disc.sorted.bam???)
#    sample.cl.disc.bam (sample.cl.disc.sorted.bam???)
#    sample.isinfo
if (! -d "$dir/bam") { 		
	$cmd = "mkdir -p $dir/bam";
	print $cmd."\n";
...
# Joe:
#  Input file copying/indexing logic done.  Note that samtools and ra.pl must
#  be in $PATH for the above code to work.  BIG NOTE: this script DELETES
#  files in $dir.  Do not make $dir point to your original files!  Original
#  files contain the ".sorted." infix.  These files are apparently re-sorted
#  into files without ".sorted." infixes and the original files are deleted.
#  These 5 files should now exist:
#    sample.disc.bam
#    sample.disc.bam.bai
#    sample.cl.disc.bam
#    sample.cl.disc.bam.bai
#    sample.isize
#
################################################################################
...
if ($step == -1 || $step == 2) {
	if (! -e "$dir/bam/$sample.ram.bam" || $force) {
		...
		# Joe: changed from original
		#my $sra = "/scratch/el114/assembly/".basename($ra);
		my $sra = "$scratch/assembly/".basename($ra);
		if (!-e $sra) {
			print "mkdir -p $scratch/assembly\n";
			# XXX: need to add an option for this
			$cmd = "cp $tea/lib/assembly/".basename($ra)."* $scratch/assembly/";
			print $cmd."\n";
			if ($exec) { system($cmd) }
...
if ( $step == -1 || $step == 4 ) {
	...
	for (my $i=1; $i<=22; $i++) { push(@chrl, "chr$i"); } push(@chrl, "chrX"); push(@chrl, "chrY"); 
		for my $chr (@chrl) {
			...
			if ($short) {
				# XXX: need to make an option for location of R code
				$cmd = "$ts R --no-save --no-restore --no-environ --args rid.disc dir=\\\"$bdir\\\" sample=\\\"$sample\\\" chr=\\\"$chr\\\" short=T < $tea/R/run.rid.r";
			} else {
				# XXX: need to make an option for location of R code
				$cmd = "$ts R --no-save --no-restore --no-environ --args rid.disc dir=\\\"$bdir\\\" sample=\\\"$sample\\\" chr=\\\"$chr\\\" < $tea/R/run.rid.r";
			}
			print $cmd."\n";
			if ($exec) { system($cmd) }
...
###############################################
# step5: merge rid per chr into one: do I need this? 
